...
static void expect_cpp_int(const Term& t, const cpp_int& expected) {
...
static void test_round_trip_big_values(Ctx& ctx) {
    ...
    Term t = cpp_int_to_term(ctx, -big);
...
static void test_constant_folding(Ctx& ctx) {
    ...
    Term ta = cpp_int_to_term(ctx, a);
    Term tb = cpp_int_to_term(ctx, b);
