...
QfSolver::QfSolver(const Ctx& ctx, const Options& opts, const Term& formula)
    : _ctx(ctx), _opts(opts) {
    STATS.begin_phase(STATS.nnf_time);
    Term f = NNFConverter(ctx)(formula);
    STATS.end_phase();
...
std::optional<bool> QfSolver::solve() {
    ...
    while (true) {
        ...
        if (g_verbosity >= 2) {
            for (auto& [c, v] : *model)
                LOG(LOG_TAG, 2, "  %s = %s", c.to_string().c_str(),
                    v.to_string().c_str());
        }
