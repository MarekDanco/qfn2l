...
// ── CheckVal ──────────────────────────────────────────────────────────────────
CheckVal::CheckVal(const Ctx& ctx, HasUninterpreted& hu,
                   const Pures& pures, const smt::SmtSolver& lia_solver)
...
std::optional<smt::Term> CheckVal::visit_leaf(const smt::Term& t) {
    if (t->is_value()) return t;
    if (is_symbolic_const(t)) {
        // Look up in the LIA model.
        // TODO: get_value may throw if t has no value; wrap in try/catch.
        try { return _lia_solver->get_value(t); }
        catch (...) { return std::nullopt; }
    }
    return std::nullopt;
}

std::optional<smt::Term> CheckVal::visit_prop(
    const smt::Term& t,
    const std::vector<std::optional<smt::Term>>& cvs)
{
    auto has_none = [&]{ for (auto& v : cvs) if (!v) return true; return false; };
    auto in_cvs   = [&](const smt::Term& x){
        for (auto& v : cvs) if (v && *v == x) return true; return false;
    };
...
std::optional<smt::Term> CheckVal::visit_complex(
    const smt::Term& t,
    const std::vector<std::optional<smt::Term>>& cvs)
{
    bool has_none = false;
    ...
    // Numeric shortcuts.
    if (is_mul(t)) {
        for (auto& v : cvs) if (v && is_zero(_ctx, *v)) return _ctx.ZERO;
    }
...
std::optional<smt::Term> CheckVal::visit(const smt::Term& t) {
    ...
    if (t->begin() == t->end()) return visit_leaf(t);
