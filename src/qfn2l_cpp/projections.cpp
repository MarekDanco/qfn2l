...
const_lb_pow(const Ctx& ctx, const smt::Term& x, int n, const smt::Term& x_val) {
    ...
    smt::Term cond = mk_and2(ctx,
        ctx.solver->make_term(smt::Le, x_val, x),
...
const_ub_pow(const Ctx& ctx, const smt::Term& x, int n, const smt::Term& x_val) {
    ...
    if (!is_neg_val(x_val)) {
        smt::Term cond = mk_and2(ctx,
            ctx.solver->make_term(smt::Le, ctx.ZERO, x),
...
smt::Term triple_to_axiom(const Ctx& ctx,
                           ...
                           const smt::Term& lhs,
                           const smt::Term& rhs) {
    return mk_implies(ctx, condition,
                      ctx.solver->make_term(smt::Le, lhs, rhs));
}
...
mod_ax_mul(const Ctx& ctx, int max_modulus,
    ...
    for (int k = 2; k <= max_modulus; ++k) {
        ...
        smt::Term ax = mk_implies(ctx, mk_and(ctx, conditions),
            ctx.solver->make_term(smt::Equal,
                ctx.solver->make_term(smt::Mod, pure, kz),
...
project_y(const Ctx& ctx,
    ...
    if (is_negative_power(y_val, y_exp))
        return {
            {mk_and2(ctx, xgt0, ub_cond), pure_res, ub_xm},
            {mk_and2(ctx, xlt0, ub_cond), ub_xm,    pure_res}
        };
    else
        return {
            {mk_and2(ctx, xgt0, lb_cond), lb_xm,    pure_res},
