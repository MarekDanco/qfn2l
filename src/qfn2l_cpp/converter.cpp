...
smt::Term NNFConverter::flip_cmp(const smt::Term& t) {
    ...
    switch (op) {
    case smt::Le: return _ctx.solver->make_term(smt::Gt, a, b);
    case smt::Ge: return _ctx.solver->make_term(smt::Lt, a, b);
    case smt::Lt: return _ctx.solver->make_term(smt::Ge, a, b);
    case smt::Gt: return _ctx.solver->make_term(smt::Le, a, b);
...
smt::Term NNFConverter::to_nnf_inner(const smt::Term& expr, bool negate) {
    ...
    if (is_distinct(expr)) {
        ...
        for (size_t i = 0; i < chs.size(); ++i)
            for (size_t j = i+1; j < chs.size(); ++j)
                neqs.push_back(mk_not(_ctx,
                    _ctx.solver->make_term(smt::Equal, chs[i], chs[j])));
    ...
    if (is_forall(expr) || is_exists(expr)) {
        ...
        smt::Term new_body = inner.convert(
            do_substitute(_ctx, body, bound_vars, fresh), negate);
    ...
    if (is_eq(expr) && num_children(expr) == 2) {
        ...
        if (a->get_sort() == _ctx.bool_sort) {
            smt::Term rewrite = mk_and2(_ctx,
                mk_implies(_ctx, a, b), mk_implies(_ctx, b, a));
