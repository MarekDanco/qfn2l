...
\begin{frame}{Main loop}
	Given a NIA formula $\varphi$:
	\begin{enumerate}
		\item $\widehat\varphi, P \gets \textsc{Purify}(\varphi)$
		\item $\mathcal{A}\gets\emptyset$ \hfill\emph{(refinement axioms)}
		      \pause
		\item \textbf{loop}
		      \begingroup
		      ...
		      \begin{enumerate}
			      \setcounter{enumii}{3}
			      \item $\mathit{res}\gets\textsc{Solve}(\widehat\varphi\land\bigwedge\mathcal{A})$
			      \item \textbf{if} $\mathit{res}=\mathit{unsat}$ \textbf{then return}
			            \alert{\textsc{unsat}}
			            \pause
			      \item $\mu\gets\textsc{Model}(\mathit{res})$
			      \item $\mathcal{N}\gets\textsc{Check}(\varphi,\widehat\varphi,P,\mu,\mathcal{A})$
			            \hfill\emph{(check $\mu$ against NIA semantics)}
			            \pause
			      \item \textbf{if} $\mathcal{N}=\emptyset$ \textbf{then return}
			            \alert{\textsc{sat}}, $\mu$
			            \hfill\emph{($\mu$ is also model under NIA)}
...
\begin{frame}{Example run}
	...
	\begin{enumerate}
		...
		\item<3-> NIA check: $\mu(p) \neq \mu(x^2) \Rightarrow \alert{\text{$\mu$ is spurious}}$\\
