...
\begin{frame}{Why care? Verifying smart contracts}
	...
	\begin{block}{BeautyChain (BEC) token hack, 2018}
		...
		$\begin{array}{l}
				...
				\right\}\ \alert{\text{attacker's pick}} \\
					\hphantom{x}\texttt{amount}\text{: total debited from the attacker's balance}
			\end{array}$
		\[
	...
	\pause
	\begin{itemize}
		\item Correctness can be verified algorithmically!
...
\begin{frame}{Main loop}
	...
	\end{center}
	\begin{itemize}
		\item $\widehat\varphi\land\bigwedge\mathcal{A}$ is an
...
\begin{frame}{Checking $\mu$ against NIA semantics}
	LIA solver returns model $\mu$:
	\begin{block}
		\small
		\begin{enumerate}
			\item $I\gets\textsc{Implicant}(\widehat\varphi,\mu)$
			      \hfill\emph{(a part of $\widehat\varphi$ that suffices: $\mu\models I\models\widehat\varphi$)}
			      \pause
			\item $R\gets\{\pure{t}\in I \mid \mu(\pure{t})\neq\mu(t)\}$
			      ...
			      \medskip
			      \begin{center}
				      Example:\quad $\mu(\pure{x^3})=5$ \quad but \quad $\mu(x)=2$
			      \end{center}
			      \medskip
			      \pause
...
\begin{frame}{Example}
	...
	\begin{enumerate}
		\small
		\item<1-> Purify: introduce $\alert<1>{\pure{x^2}}$
		      \[
			      \hspace*{-\leftmargin}\makebox[\textwidth][c]{$\widehat\varphi:\quad
					      \alert<1>{\pure{x^2}}\leq x\land x\geq 2$}
		      \]
		\item<2-> LIA solve: $\mu=\{x\mapsto 2,\ \pure{x^2}\mapsto 1\}$
		\item<3-> NIA check: $\mu(x^2) \neq \mu(\pure{x^2}) \Rightarrow \alert{\text{$\mu$ is spurious}}$\\
		      $I=\{\pure{x^2}\leq x,\ x\geq 2\}$, $R=\{\pure{x^2}\}$
		\item<4-> add axioms for $\pure{x^2}$, among them \textit{Linear Lower Bound}:
		      \[
			      \hspace*{-\leftmargin}\makebox[\textwidth][c]{$x\geq 2 \;\Rightarrow\; \pure{x^2}\geq 2x$}
		      \]
		\item<5-> LIA solve: $2x\leq \pure{x^2}\leq x$ gives $x\leq 0$, contradicting
		      $x\geq 2$
...
\begin{frame}{Quick reminder}
	\begin{itemize}
		...
		\item The model is \alert{spurious} $\Rightarrow$ it satisfies $\widehat\varphi$,
		      ...
		      %       \begin{itemize}
		      % 	      \item \emph{tight} axioms cut number of iterations
		      % 	      \item \emph{cheap} axioms keep each iteration fast
		      % \end{itemize}
	\end{itemize}
	\textbf{Goal}: linear axioms that make $\pure{t}$ track $t$ more closely and \alert{rule out}
...
\begin{frame}{Bounding mixed powers $\pure{x^ky^l}$}
	...
	\begin{itemize}
		\item Linearize $x^k$ via the secant bound; treat $y^l$ constant
		\item e.g.
		      \[
		      ...
		      \]
		\vskip 2 em
		\item Symmetric construction: fix $x^k$ constant, linearize $y^l$
		      instead
		\item These axioms form secant planes
	\end{itemize}
\end{frame}
	...
	\begin{itemize}
		...
		\item Four configurations:
		      \begin{itemize}
			      ...
			      \item {+}\textsc{Congruence}: pures with equal arguments are equal
			      \item {+}\textsc{Congruence}\,$-$\,\textsc{Model-Fix}: congruence on, model-fix off
		      \end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}{Results}
	\begin{center}
		...
		\begin{tabular}{lrr}
			...
			{\solver}                     & {14\,011}                & {585}                          \\
			{\solver{}+\textsc{Congruence}}   & {12\,421}                & {587}                          \\
			{\solver{}$+$\textsc{Congruence}\,$-$\,\textsc{Model-Fix}}
			                              & {11\,473}                & {587}                          \\
			\midrule
			Z3                            & 19\,205                  & 118                            \\
			MathSAT5                      & 16\,377                  & 171                            \\
...
\begin{frame}{Per-family picture}
	...
	\begin{itemize}
		...
		\item Those are shallow (usually $x\cdot y$), huge (hundreds of pures), with small solutions
			($|\mu(x)|$ < 10, for most $x$)
	\end{itemize}
\end{frame}
...
\begin{frame}{Conclusions and future work}
	\begin{block}{Summary}
		\begin{itemize}
			...
			\item competitive overall, \alert{dominant} on MathProblems benchmarks
		\end{itemize}
	\end{block}
	\begin{block}{Future work}
		\begin{itemize}
			\item part of our project on quantified NIA
			\item preprocessing for small-model sat instances
