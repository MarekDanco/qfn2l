...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.9\linewidth]{figures/cactus}
    \caption{Instances solved within a given wall-clock time limit.\label{fig:cactus}}
\end{figure}
...
\emph{Remark on uninterpreted functions.}
\solver targets QF\_NIA and does not handle uninterpreted functions (UF) natively.
This is not a fundamental limitation: the Ackermann encoding~\cite{ackermann54} reduces
QF\_UFLIA to QF\_LIA by replacing each UF application with a fresh integer variable and
adding congruence axioms of the form
$x_1 = y_1 \land \cdots \land x_n = y_n \implies f(x_1,\ldots,x_n) = f(y_1,\ldots,y_n)$.
Hence UF over integers does not add expressive power beyond plain LIA.
